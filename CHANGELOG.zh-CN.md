# 更新记录

面向用户的公开变更。版本号遵循语义化版本规范。

## [0.1.5] - 2026-09-04

### 新增

- `EditBuildingElement` 新增支持柱（高度、位置）、梁（起点、终点）、板（厚度、标高）的字段编辑，延续墙与门窗的精确字段掩码和回读契约。
- 新增 `MultiplyElements` 命令，按显式 GUID 对墙、柱、梁、板、屋顶、网面、变形体执行受控向量或双向阵列复制；强制 dry-run 与确认，单撤销事务执行，全量返回新 GUID，失败时报告回滚清理结果。
- 新增只读 `GetAttributes` 命令，盘点建筑材料、复合材料、表面、填充、笔号、线型、复合截面、图层组合和区域类别。
- 新增只读 `GetDocumentationElements` 命令，盘点标注、文本、标签、详图、工作图、剖面、立面元素；剖面／立面标记几何继续复用 `GetSectionMarkers`。
- 新增 `CreateText` 命令，以内容、锚点位置和可选字号、笔号、楼层创建最小原生单行文本，创建后原生回读；实际创建保持预览与确认双门槛，回读不一致时回报清理结果。
- 新增 `CreateDimension` 命令，以恰好两个测量点创建最小原生静态线性尺寸，支持可选尺寸线位置、单一关联元素和楼层；角度、半径、标高、链式、自定义样式和文本保持不支持，实际创建保持预览与确认双门槛，回读不一致时回报清理结果。
- 新增 `CreateLabel` 命令，以单行文本和锚点创建最小原生文字标签，支持可选引线点、单一父元素和楼层；符号标签、GDL label part、富文本、多父级和自定义模板保持不支持，实际创建保持预览与确认双门槛，回读不一致时回报清理结果。
- 新增 `ChangeStorySettings` 命令，修改一个既有楼层的项目绝对标高，或该楼层到上方相邻真实楼层的层高。命令保持预览与确认双门槛，执行后校验完整楼层快照，读回不一致时回报 best-effort 恢复结果，并拒绝 Teamwork 项目。
- 新增 `DeleteStory` 命令，仅在同时提供精确楼层索引和预期名称时删除一个既有楼层。默认预演，实际删除需显式确认；执行后校验完整楼层快照，恢复被删除层上方标高，并显式报告不可回滚状态。
- 新增 `TrimElements` 命令，完成首轮原生修剪：一个墙目标与一个屋顶操作件，支持 KeepInside / KeepOutside。其他构件组合显式失败，不做静默降级。
- 新增 `CreateSolidOperation` 命令，完成首轮原生实体运算：两个变形体的 Subtract / Add，并对结果做独立回读。其他组合与运算类型显式失败。

### 修复

- 屋顶几何读回现在返回模型坐标系中的完整闭合 PlaneRoof 轮廓。此前轮廓以 pivot 局部坐标保存，且只把第一个点转换回模型坐标，几何比较和复制工作流可能看到偏移或不完整轮廓。
- 楼梯规则预检对踢面高度、踏面宽度和 `2R+G` 改用 1 mm 容差判断，不再使用浮点严格相等；既避免有效原生楼梯因舍入误差被拒绝，也继续对真实规则冲突显式失败。

## [0.1.4] - 2026-09-03

### 新增

- 新增按 GUID 移动和编辑既有构件的命令：`MoveBuildingElements` 可平移墙、柱、梁、板、对象、灯具、区域、网面和变形体，也可移动窗和门并保持其嵌在原宿主墙内；`EditBuildingElement` 可修改墙的高度／厚度／端点、窗和门的宽高与窗台、柱和梁的尺寸，且只置位实际请求的字段，未请求的值不会漂移。
- `CreateWall`、`CreateColumn`、`CreateObject`、`CreateLamp` 新增可选 `absoluteZ`。楼层由项目楼层结构解析，并在响应中回显解析结果 `resolvedFloorIndex`／`resolvedStoryLevel`，因此按已知标高放置构件不再需要先切换激活楼层。
- `CreateStory` 可在既有楼层上方或下方插入新楼层，预览优先且需 `confirmRequired`。它只做插入：不删除楼层，也不修改既有楼层的标高。Archicad 无法撤销楼层结构变更，因此该命令在执行后会校验完整的楼层快照。
- `SwitchStory` 现已作为 `set_active_story` 工具提供。响应回报 `previousStoryIndex`，便于任务结束后显式恢复原激活楼层。
- 镜像与旋转支持显式传入 GUID 列表，不再依赖 Archicad 中当前恰好选中了什么。
- 删除超过 50 个构件时，须用 `acknowledgeCount` 回抄预演所报出的数量。`confirmRequired` 是常量，模板设置一次即可长期携带；而该数值只能从本次预演得知，因此它能证明预演确实被读过。
- 新增四个跨工程协作工具：列出运行中的 Archicad 实例及各自打开的工程；从一个实例采集构件快照；比较两份快照；把快照回放到另一个实例。这四个由本地 Workbench Server 承载而非 Add-On —— 扫描其它实例这件事，任何单个实例自身都做不到。
- 快照比较会报出「仅 A 有」「仅 B 有」，以及配对构件在几何、属性、分类、图层、楼层上的不一致，并按类型给出计数，使「这边 11 个变形体、那边没有」这类差异一眼可见。构件标识是工程局部的、不能用于跨工程配对，因此按类型与楼层分组后取容差内最近位置匹配；报告随附所用容差与配对策略，使任何结论都可复核。全程不使用浮点精确相等。
- 快照回放支持墙、柱、梁、板、区域的几何，并显式映射楼层。图层、分类和构件属性可作为快照元数据保存，但回放不会恢复它们；预览会标记 v0.1.4 不支持属性回放。
### 修复

- 墙工具默认与上层关联时，请求的 `height` 会被忽略并静默替换为层高。现已按显式高度创建；此前 2 米、4 米、12 米都会得到 3 米高的墙。
- 墙体此前恒偏绘制方向左侧 90 度，原因是参考线沿用工具设置而非中心线。现已按传入的线创建。
- 窗台高度写入了错误的字段因而完全不生效。现已改用女儿墙高度字段，并校验不超过墙顶；开洞会继承宿主墙所在楼层，修复了宿主墙不在激活楼层时窗落错层的问题。
- `set_active_story` 的 `dryRun` 与 `confirmRequired` 参数在到达 Add-On 之前被剥除，导致它只能预演、从未真正切换楼层。
- `create_stair` 不再回报与实际楼梯不符的踏面深度。该标量字段并不驱动 Archicad 的楼梯几何，且无论请求什么值回读都是一个固定值，对其断言没有意义。现在该请求会被显式判为不支持并说明原因，响应改为回报 `resolvedGoing`（由基线长度与踏步数算得），即楼梯实际的踏面深度。
- 复制构件现在是全成或全不成。此前读不出的源构件或类型不支持的源构件会被静默跳过，于是复制可能报成功却少建了构件。现在所有源构件在创建任何东西之前先全部校验，整批在一个可撤销命令内执行，中途失败会回滚已创建的部分并回报清理结果。
- 复制构件此前每个构件产生一个撤销步骤，撤销一次复制操作需要按等同于构件数量的 Ctrl+Z。现在整批只是一个撤销步骤。
- 复制数量上限此前只在构件来自 Archicad 选择集时生效，显式传入 GUID 会完全绕过。现由 Add-On 对两条路径统一强制。
- 删除构件此前完全没有数量上限，而可逆的复制却限 9 个。现在删除限每次 200 个，复制限 500 个，与批量创建一致。
- 快照回放可能把它创建的全部构件留在模型里。它的回滚把所有已创建构件放在一次调用里提交删除，而 Add-On 在超过单次上限时会拒绝，于是创建超过 200 个构件的回放一个也没清理掉。现在回滚按块进行，单块失败不再放弃其余块，剩余未清理的构件会被计数并报出。
- 回放校验不再因目标使用默认图层、默认分类或属性值不同而失败。校验只比较构件类型、几何与映射后的目标楼层，与公开的几何复现边界一致。
- 删除预演此前只列出 GUID，人无法核对。现在预演与执行结果都会回报每个构件的类型、图层和楼层，并按类型／图层／楼层给出分布汇总，便于核对逐条读不完的大批量。
- `POST /api/undo` 声称可以撤销上一步操作，但它调用的 Archicad 命令并不存在，因此永远失败却返回形似成功的响应。现改为返回 501，并说明 Archicad JSON API 未提供撤销命令、撤销请在 Archicad 中按 Ctrl+Z。

## [0.1.3] - 2026-08-22

### 修复

- 板、屋顶、区域的创建现在接受闭环轮廓（末点重复首点）。此前这类输入会产生零长边，Archicad 直接拒绝整个元素并只回抛裸错误码。而闭环恰好就是几何读取返回的形状，因此「读回轮廓再创建」现在可以正常往返。
- 区域创建此前完全不可用 —— 任何多边形都会失败。现已按 Archicad 要求的索引方式写入顶点、补齐多边形头部信息，并将区域标记为手动创建，使传入的轮廓被真正采用，而不是从周围墙体自动探测。
- 区域的 `zoneName` 和 `zoneCategory` 现在会真正写入元素。此前两者被接受并原样回显，但实际被丢弃。类别不存在时现在显式失败，不再静默沿用默认类别。
- 显式指定板厚度时现在会生效。项目默认板为复合结构时，标量厚度此前被静默忽略，而响应仍回显请求值。
- 复制元素现已支持变形体与网面；此前选中这两类会报「不支持的元素类型」。
- 退化的多边形输入（去重后不足三点、或存在连续重复点）现在返回明确的校验信息，而不是裸的 Archicad 错误码。
- MCP 集成面板不再误报连接状态。客户端身份改为取自 MCP `initialize` 握手，因此以编辑器扩展形式运行的宿主（如 CodeBuddy）能被正确识别。仅完成配置但未握手的宿主显示为「已配置」而非「已连接」。
- 公开仓 CI 现在只从原本私有的顶层 `tests/` 目录导出并放行 `tests/test-command-capabilities.js`，既保证命令安全检查可运行，也不会扩大公开源码边界。

### 新增

- 新增只读端点，列出所有正在运行的 Archicad 实例及各自打开的工程，为需要两个实例的跨文件工作流提供基础。

### 已知限制

以下功能已实现但**尚未在 Archicad 上验证**。本版本请勿依赖，验证已安排到下一版本。

- 按收藏夹或项目图库选择 GSM 对象，包括对象创建的 `objectRef` 参数与对象库目录查询。
- 视图截图的 `targetViewName` 参数。
- 门、窗、楼梯的读写回归覆盖。

## [0.1.2] - 2026-08-17

### 修复

- 安装程序现在会在所有本地固定磁盘上查找 Archicad，不再只查 `C:` 和 `D:`，并额外搜索 `ProgramFiles(x86)` 与 `ProgramW6432` 目录。
- 若机器上已有 MEPBridge Add-On 目录，安装程序会复用原位置，重复安装不再在不同 Add-On 目录下留下两份副本。
- 安装程序会检测 Node.js，缺失时给出警告。Add-On 本身仍会正常安装、Archicad 命令仍可使用；仅 Workbench Server、UI 和 MCP Server 需要 Node.js 18 或更高版本。
- `Install-MEPBridge.cmd` 不再隐藏交互式的 Archicad 目录询问，Archicad 装在非默认位置的用户可以正常回答该提问。
- ApplyFavorite、AssignClassification、CreateFromFavorite、SaveFavorite、SetLayerBatch 的 `dryRun` 与 `confirmRequired` 参数在到达 Add-On 之前被剥除，导致 Add-On 退回预演模式：写入实际未发生却仍报成功。现已修正，五个命令均正常传递这两个参数。
- GetViewMap 此前恒返回空列表，原因是它枚举的是发布集而非直接请求各个导航器视图图。现改为按 map id 读取公共视图图与我的视图图，并把打不开的视图图记入 `skippedMaps`，使「确实没有视图」与「查询失败」可以区分。
- CreateWall 现在会把用户传入的 `thickness` 与 `height` 写入元素，不再静默丢弃。此前 `ACAPI_Element_GetDefaults` 后未置位元素 mask，Archicad 沿用墙工具默认厚度（0.3 m）与高度（3.0 m），忽略请求值。命令现切换为 `API_BasicStructure` 使 `thickness` 字段生效，并对修改的几何字段置位 mask。
- ChangeElementGeometry、ChangeStairGeometry、ChangeOpeningGeometry 此前向 `ACAPI_Element_Change` 传入空元素 mask，Archicad 将其视为「不改动任何字段」，导致 Wall/Column/Beam/Stair/Door/Window 的几何修改被静默忽略。现改为仅对调用方明确请求的字段精确置位，不再使用 `SETFULL`；修改墙厚时还会把复合/截面墙切换为 `API_BasicStructure`，并同步写回当前建材，使标量厚度真正生效。
- ApplyFavorite 与 CreateFromFavorite 的 dry-run 响应现新增显式 `applied: false` 字段及 `note` 提示，说明需 `confirmRequired=true` 才会真正应用。此前 dry-run 仅返回 `mode: "dry-run"`，无布尔标志，调用方可能误判为已成功写入。

### 变更

- 安装指南补充 Node.js 前置要求、自定义安装路径，以及完整的手动安装分步说明，涵盖管理员权限、如何定位 Archicad 目录、如何区分 Archicad 28 与 29、旧版本处理和安装结果验证。
- 注册命令数与 descriptor 数改由单一的 `ai-adapter/command-boundary.json` 提供，发布门禁、公开仓校验、CI 与打包脚本统一读取该文件，不再各自保存一份副本。
- 命令示例不再包含硬编码的 MEP 系统名称或属性索引。这些标识跨项目、跨语言、跨 Archicad 版本并不稳定，省略后由 Add-On 使用域默认系统；并新增静态检查防止回归。
- Archicad 实机回归改为校验返回内容而非仅校验字段存在，并新增收藏夹、批量图层、分类三类写入命令的端到端覆盖，每项测完都还原自身改动。
- 内部命令指南补齐全部 74 个已注册命令。此前只有 52 个，收藏夹、截面、视图列表、分类、图层命令与项目/楼层查询均缺失。该文档的命令计数改为以 `ai-adapter/command-boundary.json` 为准。
- 公开仓校验器遇到含孤立百分号的 Markdown 链接时不再中断整轮校验，而是将该链接报为失效链接。

## [0.1.1] - 2026-07-28

### 新增

- 13 个新 C++ 命令：GetFavorites、GetFavorite、CreateFromFavorite、ApplyFavorite、SaveFavorite、GetProfiles、GetViewMap、GetSectionMarkers、GetClassifications、GetLayers、FindElementsByProperty、AssignClassification、SetLayerBatch。
- CreateWall、CreateColumn、CreateBeam 新增可选 `profileGuid` 参数，支持截面轮廓选择。
- Workbench Server 接入语义索引主链，用于自然语言命令匹配。
- README 新增示例预览章节，含演示视频和执行结果截图。
- 新增中文更新记录（`CHANGELOG.zh-CN.md`）和安装说明中的双语版本更新指引。

### 变更

- C++ 命令数从 61 增至 74；descriptor/MCP 工具数从 59 增至 72。
- CopyElements 空 sourceGuids 处理三层修复：resolver 在 `dryRun=true` 且 `sourceGuids` 为空时跳过选择集兜底；C++ schema 移除 `minItems:1`；C++ `Execute()` 对空 GUID 返回 dry-run 预览（C++ 部分需重编 APX）。
- 入门 shell 模板从 CopyElements（依赖 GUID）改为 CreateWall×8 + CreateSlab×2（10 步，自包含坐标，跨项目可用，不依赖 GUID）。
- MCP 状态显示支持多配置路径数组检测 CodeBuddy、WorkBuddy、Codex、Claude Code 四个平台；支持每个候选的自定义标记。
- Claude Code MCP 插件安装路径修正为 `~/.claude.json` 顶层 `mcpServers`（非 `~/.claude/mcp.json`）。
- 刷新 README 文案（移除 openBIM 框架、收紧安全与免责声明措辞、更新作者署名、补充第三方插件声明）并用示例预览替换工作流程图。
- 示例楼板和示例桥架模板坐标重置为源构件原生位置（无 X+20m 偏移），匹配当前 AC28 0 层读取/重建回归数据。
- 用户模板任务名在列表、分组、选择器、管理卡片和重放对话框中改为纯文本渲染；移除每个模板不一致的图标和热点标记。
- 安装包语言决定 Workbench 初始语言、入门资产和内置区域知识库；保留手动切换语言能力。
- 公开 `VERSION` 版本事实源，为每次公开 PR 与 `main` 更新引入自动化版本闸门。
- 强制 PR 清单，覆盖更新记录、版本影响、双语文档与 Release 资产影响。
- 每次公开仓库更新必须在 `[未发布]` 下追加简洁条目。
- 产品版本变更必须递增语义化版本号，并同步更新记录、中英文公开指南和 Server/UI 包元数据。

### 修复

- CopyElements 空 sourceGuids 三层拦截：resolver 在 `dryRun=true` 且 `sourceGuids` 为空时不再读 Archicad 选择集；C++ schema `minItems:1` 移除；C++ `Execute()` 对空 GUID 返回 dry-run 预览成功。
- MCP 状态显示只显示 CodeBuddy，WorkBuddy 和 Codex 已打开却不显示。修复方式：新增 WorkBuddy、支持每个平台多配置路径、移除误报匹配器。
- 修正源码、descriptor、MCP 和文档之间的命令数偏差。
- 移除公开发布范围内的过时重复 UI 产物和 source map。
- 修正 AC28/AC29 包路径。
- 切换到仅对话窗口时保留会话历史和执行状态。

### 已知边界

- `SwitchStory` 和 `ChangeStairGeometry` 是注册 C++ 命令，但在 v0.1.1 未作为 descriptor/MCP 工具发布。
- `CopyElementsCommand.cpp` schema 和 `Execute()` 改动需重编 APX 才能使 dryRun 占位跨项目可用；当前已用 Create 型模板规避。
- `GetViewMap` 在当前测试项目返回 0 视图（项目无 View Map 项，非 bug）。
- `CreateColumn`/`CreateBeam` 的 `profileGuid` 解析但不应用（memo 多段结构，后续增强）。
- 公开源码仓库不包含 Graphisoft DevKit，不承诺可复现的官方 APX 构建。
- 用户对项目备份、输入审查和合规性自行负责。

## [0.1.0] - 2026-07-20

### 新增

- Archicad 28 与 Archicad 29 Windows Add-On。
- 61 个注册 C++ 命令、59 个 descriptor/MCP 工具。
- 通用建筑构件与 MEP 构件的创建、查询、编辑、属性、选择和项目环境操作。
- 本地 Node.js Workbench Server 与 React UI。
- Manual 与 CollabAI 模式，含确认策略与结果回读。
- 兼容主流 MCP 宿主的 MCP stdio Server。
- 本地模板、自定义命令、知识库、学习记忆、主动建议和审计日志。
- Windows 安装与卸载入口。
- 发布清单与每文件 SHA256 校验和。

### 变更

- 公开版本号统一为 `0.1.0`。
- AC28 与 AC29 使用独立资源文件和菜单注册。
- Server 默认监听 `127.0.0.1`；暴露到网络需显式 `HOST` 覆盖。
- 运行时用户数据默认保存在 `%APPDATA%\MEPBridge`。
- MCP 工具数量由 descriptor 动态生成。
- 公开仓库布局区分用户文档、打包源文件和内部技术资源。

### 修复

- 本周期无。

### 已知边界

- `SwitchStory` 和 `ChangeStairGeometry` 是注册 C++ 命令，但在 v0.1.0 未作为 descriptor/MCP 工具发布。
- 部分 AC28/AC29 实现细节可能在后续更新中继续收敛。
- 公开源码仓库不包含 Graphisoft DevKit，不承诺可复现的官方 APX 构建。
- 用户对项目备份、输入审查和合规性自行负责。

---

版权所有 (c) 2026 Zuxai Z. 基于 MIT License 授权。
